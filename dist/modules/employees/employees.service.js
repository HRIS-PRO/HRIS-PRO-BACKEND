...
class EmployeesService {
    ...
    async createEmployee(data) {
        ...
        // 2. Create Employee
        const [employee] = await this.db.insert(schema_1.employees).values({
            userId: user.id,
        ...
        // 4. Send Welcom Email
        await (0, zepto_1.sendEmail)(data.workEmail, 'Welcome to HRIS.Pro - Your Account Credentials', `
    ...
    async assignEmployeeRole(employeeId, appName, roleName) {
        ...
        if (existing) {
            // Update
            const [updated] = await this.db.update(schema_1.userRoles).set({
                role: roleName
            }).where((0, drizzle_orm_1.eq)(schema_1.userRoles.id, existing.id)).returning();
            return updated;
        }
        else {
            // Insert new
    ...
    async revokeEmployeeRole(employeeId, appName) {
        const employee = await this.db.query.employees.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.employees.id, employeeId),
            columns: { userId: true }
        });
