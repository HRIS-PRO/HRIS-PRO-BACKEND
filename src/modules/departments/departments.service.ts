...
import { departments, employees, users, userRoles } from '../../db/schema';
import { db } from '../../db';
...
export class DepartmentsService {
    ...
    async getEligibleHeads() {
        ...
        let eligibleQuery = this.db.select().from(employees);
        if (headIds.length > 0) {
            eligibleQuery = eligibleQuery.where(notInArray(employees.id, headIds)) as any;
        }
    ...
    async createDepartment(data: any, rootUserId?: string) {
        ...
        if (finalHeadId) {
            ...
            const emp = await this.db.query.employees.findFirst({
                where: eq(employees.id, finalHeadId)
            });
            ...
            const user = await this.db.query.users.findFirst({
                where: eq(users.id, rootUserId)
            });
            finalHeadName = user?.email || 'System Owner';
        }

        const [newDept] = await this.db.insert(departments).values({
            name: data.name,
    ...
    async updateDepartment(id: string, data: any, rootUserId?: string) {
        let finalHeadId = data.headId;
        ...
        const [updatedDept] = await this.db.update(departments)
            .set(updateData)
